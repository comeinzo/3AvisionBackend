...
def fetch_data(table_name, x_axis_columns, filter_options, y_axis_column, aggregation, db_name, selectedUser, calculationData):
    ...
    # # Check if x_axis column is a date column and apply date grouping
    # original_x_axis_col = None
    # if x_axis_columns and len(x_axis_columns) > 0:
    #     x_axis_col = x_axis_columns[0]
        
    #     # Try to convert to datetime to check if it's a date column
    #     try:
    #         temp_date_check = pd.to_datetime(temp_df[x_axis_col], errors='coerce')
            
    #         # If most values are valid dates, treat as date column
    #         if temp_date_check.notna().sum() > len(temp_df) * 0.5:
    #             print(f"Detected date column: {x_axis_col}")
    #             original_x_axis_col = x_axis_col
                
    #             # Convert the column to datetime
    #             temp_df[x_axis_col] = pd.to_datetime(temp_df[x_axis_col], errors='coerce')
                
    #             # Apply date grouping if specified
    #             if date_grouping:
    #                 date_grouping = date_grouping.lower()
                    
    #                 if date_grouping == 'year':
    #                     # Create a new column for year grouping
    #                     temp_df[f'{x_axis_col}_grouped'] = temp_df[x_axis_col].dt.year
    #                     x_axis_columns = [f'{x_axis_col}_grouped']
                        
    #                 elif date_grouping == 'quarter':
    #                     # Create quarter grouping (Q1-2024, Q2-2024, etc.)
    #                     temp_df[f'{x_axis_col}_grouped'] = (
    #                         'Q' + temp_df[x_axis_col].dt.quarter.astype(str) + 
    #                         '-' + temp_df[x_axis_col].dt.year.astype(str)
    #                     )
    #                     x_axis_columns = [f'{x_axis_col}_grouped']
                        
    #                 elif date_grouping == 'month':
    #                     # Create month grouping (2024-01, 2024-02, etc.)
    #                     temp_df[f'{x_axis_col}_grouped'] = temp_df[x_axis_col].dt.to_period('M').astype(str)
    #                     x_axis_columns = [f'{x_axis_col}_grouped']
                        
    #                 elif date_grouping == 'week':
    #                     # Create week grouping (Week 1-2024, Week 2-2024, etc.)
    #                     temp_df[f'{x_axis_col}_grouped'] = (
    #                         'Week ' + temp_df[x_axis_col].dt.isocalendar().week.astype(str) + 
    #                         '-' + temp_df[x_axis_col].dt.year.astype(str)
    #                     )
    #                     x_axis_columns = [f'{x_axis_col}_grouped']
    ...
    #                     temp_df[f'{x_axis_col}_grouped'] = temp_df[x_axis_col].dt.date.astype(str)
    #                     x_axis_columns = [f'{x_axis_col}_grouped']
                        
    #                 elif date_grouping == 'day_of_week':
    #                     # Group by day of week (Monday, Tuesday, etc.)
    #                     temp_df[f'{x_axis_col}_grouped'] = temp_df[x_axis_col].dt.day_name()
    #                     x_axis_columns = [f'{x_axis_col}_grouped']
                        
    #                 elif date_grouping == 'hour':
    #                     # Group by hour
    #                     temp_df[f'{x_axis_col}_grouped'] = temp_df[x_axis_col].dt.hour
    #                     x_axis_columns = [f'{x_axis_col}_grouped']
                        
    #                 print(f"Applied {date_grouping} grouping on date column")
                
    #     except Exception as e:
    #         print(f"Column {x_axis_col} is not a date column or date conversion failed: {e}")
    
    # # ========== END OF NEW DATE FILTERING LOGIC ==========
    ...
    if x_axis_columns and len(x_axis_columns) > 0:
        ...
        try:
            ...
            if temp_date_check.notna().sum() > len(temp_df) * 0.5:
                ...
                # Apply date grouping if specified
                if date_grouping:
                    # Ensure date_grouping is a list to handle single or multiple values
                    if isinstance(date_grouping, str):
                        date_grouping = [date_grouping]

                    # Sort by the original date column FIRST to ensure final groups are in order
                    temp_df = temp_df.sort_values(by=x_axis_col)

                    temp_grouping_cols = []
                    # Loop through each requested grouping level
                    for group in date_grouping:
                        group = group.lower().strip()
                        new_col_name = f'{x_axis_col}_{group}'
                        temp_grouping_cols.append(new_col_name)
                        
                        if group == 'year':
                            temp_df[new_col_name] = temp_df[x_axis_col].dt.year.astype(str)
                        elif group == 'quarter':
                            temp_df[new_col_name] = 'Q' + temp_df[x_axis_col].dt.quarter.astype(str)
                        elif group == 'month':
                            temp_df[new_col_name] = temp_df[x_axis_col].dt.strftime('%b')
                        elif group == 'week':
                            # Note: This is the week number of the year (1-53)
                            temp_df[new_col_name] = temp_df[x_axis_col].dt.isocalendar().week.astype(str)
                        elif group == 'day':
                            # Note: This is the day of the month (1-31)
                            temp_df[new_col_name] = temp_df[x_axis_col].dt.day.astype(str)
                        elif group == 'day_of_week':
                            temp_df[new_col_name] = temp_df[x_axis_col].dt.day_name()
                        elif group == 'hour':
                            temp_df[new_col_name] = temp_df[x_axis_col].dt.hour.astype(str)
                    
                    # Combine the parts into a single column with '-' as a separator
                    grouped_col_name = f'{x_axis_col}_grouped'
                    temp_df[grouped_col_name] = temp_df[temp_grouping_cols].agg('-'.join, axis=1)
                    
                    # Set the new combined column as the one to group by
                    x_axis_columns = [grouped_col_name]

                    print(f"Applied combined grouping {date_grouping} on date column")
...
#                     if date_grouping == 'year':
#                         # Create a new column for year grouping
#                         temp_df[f'{x_axis_col}_grouped'] = temp_df[x_axis_col].dt.year.astype(str)
#                         x_axis_columns = [f'{x_axis_col}_grouped']
...
#                     #     x_axis_columns = [f'{x_axis_col}_grouped']
#                     elif date_grouping == 'month':
#                         # Create month grouping (Jan, Feb, etc.)
#                         temp_df[f'{x_axis_col}_grouped'] = temp_df[x_axis_col].dt.strftime('%b')
#                         x_axis_columns = [f'{x_axis_col}_grouped']                   
#                     elif date_grouping == 'week':
#                         # Create week grouping (Week 1-2024, Week 2-2024, etc.)
#                         temp_df[f'{x_axis_col}_grouped'] = (
...
#                     elif date_grouping == 'hour':
#                         # Group by hour
#                         temp_df[f'{x_axis_col}_grouped'] = temp_df[x_axis_col].dt.hour.astype(str)
#                         x_axis_columns = [f'{x_axis_col}_grouped']
...
# ========== END OF NEW DATE FILTERING LOGIC ==========
    ...
    # Filter again based on x-axis - handle both original and grouped columns
    if len(options) > 0:
        filtered_df = temp_df[temp_df[x_axis_columns[0]].isin(options)]
    else:
        # If no specific filter options, use all data
        filtered_df = temp_df

    # Perform aggregation
    groupby_col = x_axis_columns_str[0]
    
    # Ensure the groupby column is valid
    if groupby_col not in filtered_df.columns:
        print(f"Warning: {groupby_col} not found in columns. Using available data.")
        filtered_df = temp_df
        groupby_col = x_axis_columns_str[0] if x_axis_columns_str[0] in temp_df.columns else temp_df.columns[0]
    
    if aggregation == "sum":
        grouped_df = filtered_df.groupby(groupby_col)[y_axis_column[0]].sum().reset_index()
    elif aggregation == "average":
        grouped_df = filtered_df.groupby(groupby_col)[y_axis_column[0]].mean().reset_index()
    elif aggregation == "count":
        grouped_df = filtered_df.groupby(groupby_col).size().reset_index(name="count")
    elif aggregation == "maximum":
        grouped_df = filtered_df.groupby(groupby_col)[y_axis_column[0]].max().reset_index()
    elif aggregation == "minimum":
        grouped_df = filtered_df.groupby(groupby_col)[y_axis_column[0]].min().reset_index()
    elif aggregation == "variance":
        grouped_df = filtered_df.groupby(groupby_col)[y_axis_column[0]].var().reset_index()
    else:
        raise ValueError(f"Unsupported aggregation type: {aggregation}")

    # Sort by date if it's a date grouping
    if date_grouping and original_x_axis_col:
        try:
            if date_grouping in ['year', 'hour']:
                grouped_df = grouped_df.sort_values(by=groupby_col)
            elif date_grouping == 'quarter':
                # Sort quarters properly
                grouped_df['sort_key'] = grouped_df[groupby_col].str.extract(r'Q(\d)-(\d{4})').apply(
                    lambda x: int(x[1]) * 10 + int(x[0]), axis=1
                )
                grouped_df = grouped_df.sort_values('sort_key').drop('sort_key', axis=1)
            elif date_grouping == 'month':
                # Sort months in proper calendar order
                month_order = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
                grouped_df['sort_key'] = grouped_df[groupby_col].map({month: i for i, month in enumerate(month_order)})
                grouped_df = grouped_df.sort_values('sort_key').drop('sort_key', axis=1)

            elif date_grouping in ['week', 'day']:
                # These are already in sortable format
                grouped_df = grouped_df.sort_values(by=groupby_col)
            elif date_grouping == 'day_of_week':
                # Sort days of week in proper order
                day_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
                grouped_df['sort_key'] = grouped_df[groupby_col].map({day: i for i, day in enumerate(day_order)})
                grouped_df = grouped_df.sort_values('sort_key').drop('sort_key', axis=1)
        except Exception as e:
            print(f"Could not sort date groupings: {e}")
