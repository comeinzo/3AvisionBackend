...
from bar_chart import (
    ...
    perform_calculation,apply_all_calculations
)
...
#             df, x_axis, y_axis = apply_all_calculations(
#                     df,
#                     calculation_data,
#                     x_axis,
#                     y_axis
...
# def apply_calculation_to_df(df, calculation_data_list, x_axis=None, y_axis=None):
#     """
#     Apply a list of calculation formulas to the dataframe.

#     Parameters:
#         df (pd.DataFrame): The input dataframe.
#         calculation_data_list (list): List of calculation dicts, each containing:
#             - calculation: str
#             - columnName: str
#             - replaceColumn: optional str
#         x_axis (list): Columns used in x-axis (optional)
#         y_axis (list): Columns used in y-axis (optional)

#     Returns:
#         df (pd.DataFrame): DataFrame with calculated columns added.
#         x_axis (list): Updated x_axis list with renamed columns.
#         y_axis (list): Updated y_axis list with renamed columns.
#     """
#     if not calculation_data_list:
#         return df, x_axis, y_axis
...
#     for calc_entry in calculation_data_list:
#         if not isinstance(calc_entry, dict):
#             print(f"❌ Skipping invalid calculation entry (not a dict): {calc_entry}")
#             continue

#         calc_formula = calc_entry.get('calculation')
#         new_col_name = calc_entry.get('columnName')
#         replace_col_name = calc_entry.get('replaceColumn', new_col_name)

#         if not calc_formula or not new_col_name:
#             print(f"❌ Skipping incomplete calculation entry: {calc_entry}")
#             continue

#         try:
#             # Apply the calculation using your modular function
#             df, x_axis, y_axis = apply_all_calculations(df, calc_entry, x_axis, y_axis)
#             print("df  ",df)

#             # Update axis columns if needed
#             if y_axis:
#                 y_axis = [new_col_name if col == replace_col_name else col for col in y_axis]
#             if x_axis:
#                 x_axis = [new_col_name if col == replace_col_name else col for col in x_axis]
...
#         except Exception as e:
#             print(f"❌ Error applying calculation for column '{new_col_name}': {e}")
...
def apply_calculation_to_df(df, calculation_data_list, x_axis=None, y_axis=None):
    ...
    for calc_entry in calculation_data_list:
        ...
        try:
            # Wrap single dict in a list for apply_all_calculations
            df, x_axis, y_axis = apply_all_calculations(df, [calc_entry], x_axis, y_axis)
        ...
        try:
            ...
            for item in raw_list:
                if isinstance(item, str):
                    try:
                        active_temp_filters.update(json.loads(item))
                    except:
                        pass # Ignore invalid JSON strings
                elif isinstance(item, dict):
                    active_temp_filters.update(item)
...
@token_required
def get_dashboard_tables():
    dashboards = request.args.get("dashboard")
    company_name = request.args.get("company_name")
    user_id = request.args.get("user_id")
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # 🚀 Single query fetches all chart_ids together
            cur.execute("""
                SELECT DISTINCT unnest(chart_ids::int[]) AS cid
                FROM table_dashboard
                WHERE file_name = ANY(%s)
                  AND user_id = %s
                  AND (%s IS NULL OR company_name = %s)
            """, (dashboards, user_id, company_name, company_name))

            chart_ids = [row["cid"] for row in cur.fetchall()]
            if not chart_ids:
                return jsonify({"tables": []}), 200

            # 🚀 Fetch tables in ONE query
            cur.execute("""
                SELECT DISTINCT selected_table
                FROM table_chart_save
                WHERE id = ANY(%s)
            ...
            tables = [r["selected_table"] for r in cur.fetchall()]

        return jsonify({"tables": tables}), 200

    except Exception as e:
        print("Dashboard tables error:", e)
        return jsonify({"error": "Failed to fetch tables"}), 500
...
#     table_name = request.args.get('table')
#     column_name = request.args.get('column')
#     company_db = request.args.get('company_name')
...
#                 ORDR BY "{column}" 
#                 LIMIT 5000
#             """.format(column=column_name, table=table_name))
...
def get_columnFilter_values():
    ...
    column_name = request.args.get('column')
    company_db = request.args.get('company_name')
    search = request.args.get('search', "")  # <-- wildcard search text
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Step 1: Count distinct rows in column
            count_query = f'SELECT COUNT(DISTINCT "{column_name}") FROM "{table_name}"'
            cur.execute(count_query)
            total_count = cur.fetchone()[0]

            # Step 2: If more than 1000, apply wildcard filtering
            if total_count > 1000:

                query = f'''
                    SELECT DISTINCT "{column_name}" AS value
                    FROM "{table_name}"
                    WHERE "{column_name}"::text ILIKE %s
                    ORDER BY "{column_name}"
                    LIMIT 200
                '''
                wildcard = f"%{search}%"
                cur.execute(query, (wildcard,))

            else:
                # Fetch full list (small column)
                query = f'''
                    SELECT DISTINCT "{column_name}" AS value
                    FROM "{table_name}"
                    WHERE "{column_name}" IS NOT NULL
        ...
        return jsonify({"values": values, "total": total_count}), 200
...
# def generate_sql_with_openai(question, schema, table_name):
#     """Generate SQL using OpenAI API with agentic reasoning."""
    
#     prompt = f"""You are an intelligent SQL agent. Convert the natural language question to a PostgreSQL query.
...
#             model="gpt-4o-mini",
#             messages=[
#                 {"role": "system", "content": "You are an expert SQL agent who generates optimized PostgreSQL queries. You think step-by-step and provide only the SQL code."},
...
#     prompt = f"""You are a helpful data analyst assistant. Based on the SQL query results, provide a natural, conversational answer to the user's question.
...
#             model="gpt-4o-mini",
#             messages=[
#                 {"role": "system", "content": "You are a helpful data analyst who explains query results in simple, conversational language."},
#                 {"role": "user", "content": prompt}
#             ],
#             temperature=0.7,
#             max_tokens=200
...
# def health_check_of_agentai():
#     """Health check endpoint."""
#     return jsonify({
#         'status': 'healthy',
#         'message': '3Avision SQL Agent API is running'
#     })
...
#     """Execute a custom SQL query directly (advanced users)."""
#     try:
#         data = request.json
...
#         # Convert DataFrame to list of dicts for JSON response
#         results_list = results_df.to_dict('records') if results_df is not None else []
#         print("print the data according to the result ",results_list)
...
# ============================================================================
# GEMINI AI FUNCTIONS (Replaces OpenAI Logic)
# ============================================================================

# def generate_sql_with_gemini(question, schema, table_name):
#     """Generate SQL using Google Gemini API with agentic reasoning."""
    
#     # We combine system instruction into the prompt for maximum compatibility across Gemini versions
...
# 3. Generate an efficient PostgreSQL query
# 4. The table name is '{table_name}'
# 5. Return ONLY the SQL query without any explanation or markdown formatting (no ```sql).
...
def health_check_of_agentai():
    ...
    return jsonify({
        'status': 'healthy',
        'message': f'3Avision SQL Agent API is running (Using {ACTIVE_MODEL_NAME})'
    })
...
def get_schema(table_name):
    ...
    try:
        schema = get_table_schema(table_name)
        if schema:
            return jsonify({'success': True, 'table_name': table_name, 'schema': schema})
        else:
...
def determine_optimal_chart(question, results_df, sql_query):
    """
    Analyzes the query and data to determine the best visualization.
    Returns a dict with chart_type, x_axis, y_axis, etc.
    """
    try:
        # Create a small sample of data so Gemini knows the types (numbers, dates, strings)
        # We convert timestamps to strings for JSON serialization in the prompt
        ...
        Data Sample (First 3 rows): {json.dumps(data_sample, default=str)}

        Task: Decide the best chart type to visualize this data.
        
        Rules:
        1. "bar": for comparing categories (e.g., Sales by Product).
        2. "line": for trends over time (e.g., Revenue by Month/Year).
        3. "pie": for parts of a whole (e.g., Status distribution).
        4. "scatter": for correlation between two numbers.
        5. "None": If data is a single number, text-only, or too complex.

        Output JSON only:
        {{
            "chart_type": "bar" | "line" | "pie" | "scatter" | "None",
            "x_axis_column": "exact_column_name_for_labels",
            "y_axis_column": "exact_column_name_for_values",
        ...
        print(f"Error in chart determination: {e}")
        return None
    ...
    try:
        ...
        for condition in conditions:
            ...
            if eq_match:
                column = eq_match.group(1)
                value = eq_match.group(2).strip().strip("'\"")
                if column not in filters: filters[column] = []
                if value not in filters[column]: filters[column].append(value)
                continue
            
            # LIKE / ILIKE
            like_match = re.match(r'(\w+)\s+I?LIKE\s+[\'"](.+)[\'"]', condition, re.IGNORECASE)
            if like_match:
                column = like_match.group(1)
